"""
SPHINCS+ Alpha Signature Size Breakdown
====================================
Reports the byte composition of a SPHINCS+ Alpha signature:
  R  ||  SIG_FORS  ||  SIG_HT

Component sizes (from spec):
  R        = n bytes
  SIG_FORS = k * (n + a*n) bytes  =  k*n*(1+a) bytes
  SIG_HT   = d * (len-1 + h/d) * n bytes
"""
...
from bench_common_alpha import run_sig_size_sphincs_alpha, N_RUNS, N_WARMUP
...
if __name__ == "__main__":
    print("SPHINCS+ Alpha Signature Size Breakdown")
    print("=" * 56)
    for label, n, w, h, d, k, t in PARAM_SETS:
        params = SphincsParamsAlpha(n=n, w=w, h=h, d=d, k=k, t=t)
        r = run_sig_size_sphincs_alpha(label, params)
        print_results(r)
    print(f"\n{'=' * 56}\nDone.")
