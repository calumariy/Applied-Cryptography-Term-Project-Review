...
def run_keygen_sphincs_alpha(label: str, params: SphincsParamsAlpha,
               n_runs: int = N_RUNS, n_warmup: int = N_WARMUP) -> dict:
    sphincs = SphincsAlpha(params, randomize=False)
...
def run_sign_sphincs_alpha(label: str, params: SphincsParamsAlpha,
             n_runs: int = N_RUNS, n_warmup: int = N_WARMUP) -> dict:
    sphincs = SphincsAlpha(params, randomize=False)
    sk, pk  = sphincs.spx_keygen()
    for _ in range(n_warmup):
...
def run_verify_sphincs_alpha(label: str, params: SphincsParamsAlpha,
               n_runs: int = N_RUNS, n_warmup: int = N_WARMUP) -> dict:
    sphincs = SphincsAlpha(params, randomize=False)
    sk, pk  = sphincs.spx_keygen()
    sig     = sphincs.spx_sign(MESSAGE, sk)
...
def run_sig_size_sphincs_alpha(label: str, params: SphincsParamsAlpha,
                                  n_runs: int = N_RUNS, n_warmup: int = N_WARMUP) -> dict:
    sphincs = SphincsAlpha(params, randomize=False)
    sk, pk  = sphincs.spx_keygen()
    sig     = sphincs.spx_sign(MESSAGE, sk)
    ...
    # SIG of Sphincs Alpha
    r_size        = params.n
    sig_fors_size = params.k * params.n * (1 + a)
    sig_ht_size   = params.d * ((params.len-1) + params.h // params.d) * params.n
