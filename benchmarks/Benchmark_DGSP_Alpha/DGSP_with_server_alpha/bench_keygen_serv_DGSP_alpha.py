"""
SPHINCS+ DGSP Keygen Benchmark (server-ed)
===================================
Measures wall-clock time for keygen across parameter sets.
Methodology:
...
from bench_common_serv_DGSP_alpha import managed_server, run_keygen_server, N_RUNS, N_WARMUP
...
def run(label: str, params: SphincsParamsAlpha,
    ...
    return run_keygen_server(label, params, n_runs, n_warmup)
...
def print_results(r: dict):
    ...
    print(f"  PK_size  : {r['pk_size']} bytes  ({r['pk_size'] * 8} bits)")


if __name__ == "__main__":
    print("SPHINCS+ DGSP Keygen Alpha (server) Benchmark")
    print("=" * 56)
