...
from MemberOpenAlpha import MemberOpen
...
SERVER_PORT = 65432
SERVER_PY   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bench_server_alpha.py")
...
"""
def start_server(params: SphincsParamsAlpha,
...
@contextmanager
def managed_server(params: SphincsParamsAlpha,
                   host: str = SERVER_HOST,
...
def new_member(params: SphincsParamsAlpha, host=SERVER_HOST, 
               port=SERVER_PORT) -> Member:
    m = Member(params, host, port)
    m.fetch_pk()
    return m

def join_member(params: SphincsParamsAlpha, username: str,
                 host=SERVER_HOST, port=SERVER_PORT) -> Member:
    m = new_member(params, host, port)
    m.join(username)
    return m

def join_member_open(params: SphincsParamsAlpha, username: str,
...
def run_keygen_server(label: str, params: SphincsParamsAlpha,
...
"""
def run_join_server(label: str, params: SphincsParamsAlpha,
                    n_runs: int = N_RUNS, n_warmup: int = N_WARMUP,
                    host=SERVER_HOST, port=SERVER_PORT) -> dict:
...
def run_resp_m_server(label: str, params: SphincsParamsAlpha,
                      n_runs: int = N_RUNS, n_warmup: int = N_WARMUP,
                      host=SERVER_HOST, port=SERVER_PORT) -> dict:
    counter = 0;
...
def run_judge_server(label: str, params: SphincsParamsAlpha,
                      n_runs: int = N_RUNS, n_warmup: int = N_WARMUP,
                      host=SERVER_HOST, port=SERVER_PORT) -> dict:
    counter = 0;
...
def run_open_server(label: str, params: SphincsParamsAlpha,
                    n_runs: int = N_RUNS, n_warmup: int = N_WARMUP,
                    host=SERVER_HOST, port=SERVER_PORT) -> dict:
...
def run_verify_server(label: str, params: SphincsParamsAlpha,
                    n_runs: int = N_RUNS, n_warmup: int = N_WARMUP,
                    host=SERVER_HOST, port=SERVER_PORT) -> dict:
    counter = 0;
...
def run_sign_server(label: str, params: SphincsParamsAlpha,
                    n_runs: int = N_RUNS, n_warmup: int = N_WARMUP,
                    host=SERVER_HOST, port=SERVER_PORT) -> dict:
    counter = 0
