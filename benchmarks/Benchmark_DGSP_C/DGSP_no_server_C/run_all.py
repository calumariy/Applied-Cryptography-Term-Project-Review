...
import bench_common_noserv_DGSP_C as bench_common
import bench_keygen_noserv_DGSP_C
import bench_open_noserv_DGSP_C
import bench_judge_noserv_DGSP_C
import bench_sig_size_noserv_DGSP_C
import bench_respM_noserv_DGSP_C
import bench_revoke_noserv_DGSP_C
...
def sweep(curr_op: str) -> list:
    ...
    for idx, (n, w, h, d, k, t, t_prime, z) in enumerate(combos, 1):
        ...
        label = f"n={n} w={w} h={h} d={d} k={k} t={t} t_prime={t_prime} z={z}"
        try:
...
def print_top(results: list, curr_op: str, n: int = 10) -> None:
    ...
    if   curr_op == "sig_size": bench_sig_size_noserv_DGSP_C.print_results(ranked[0]["raw"])
    elif curr_op == "keygen":   bench_keygen_noserv_DGSP_C.print_results(ranked[0]["raw"])
    elif curr_op == "revoke":   bench_revoke_noserv_DGSP_C.print_results(ranked[0]["raw"])
    elif curr_op == "resp_m":   bench_respM_noserv_DGSP_C.print_results(ranked[0]["raw"])
    elif curr_op == "open":     bench_open_noserv_DGSP_C.print_results(ranked[0]["raw"])
    elif curr_op == "judge":    bench_judge_noserv_DGSP_C.print_results(ranked[0]["raw"])
...
def run():
    ...
    # set_memory_limits()
    print("Welcome to param optimizer: DGSP C (serverless)")
    for op in ALL_OPS:
        print(f"\nCurrently running: {op}")
        results = sweep(op)
        if results:
            print_top(results, op, n=10)
            file_path = os.path.join(script_dir, f"results_{op}_dgsp_noserv_c.csv")
