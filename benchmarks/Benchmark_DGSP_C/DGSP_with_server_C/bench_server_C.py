...
ADDED PROTOCOLS
  ...
  not initialised.
Run:
    python bench_server.py 127.0.0.1 65432
"""
...
def main() -> None:
    ...
    if len(sys.argv) < 3:
        print("Usage: python ben_server_C.py <host> <port> [n] [w] [h] [d] [k] [t] [t_prime] [z]")
        sys.exit(1)
    ...
    t_prime = int(sys.argv[9]) if len(sys.argv) > 9 else 8
    z = int(sys.argv[10]) if len(sys.argv) > 10 else 2
